...
const AdminProductForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        price: '',
        description: '',
        catagory: '', // Note: API uses 'catagory' spelling
        discount: 0,
        countInStock: 0,
        brand: '',
        image: null // Can be File object or URL string (if not updated)
    });
    ...
    useQuery({
        ...
        onSuccess: (data) => {
            setFormData({
                ...
                brand: data.brand || '',
                // If it's a string, we keep it as 'image' prop but DON'T send it in form data as 'image' file
                // unless we want to support sending URL string (which backend supports now too)
                // But generally for file input we keep it separate.
                image: data.image
            });
            if (data.image) {
    ...
    const mutation = useMutation({
        mutationFn: async (data) => {
            // Need to construct FormData here because we are sending a file
            ...
            if (data.image instanceof File) {
                ...
                // If user typed a URL (if we support that input) 
                // OR if it's existing image and we want to keep it (backend logic handles this but safer to not send 'image' field if not changing, unless we explicitely send URL)
                // Current backend logic: if req.file exists, uses it. Else if req.body.image exists, uses it.
                // So we can append it if it's a string.
                formData.append('image', data.image);
                        ...
                        <div className="col-span-2">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex-1">
                                    ...
                                    {/* Optional: Add URL input fallback if needed, but file is preferred now */}
                                </div>
                            </div>
                        </div>
                    </div>
